 ...
 */
  const startScript = () => {
    ...
    console.log(CLI.constructor.name);
  ...
  const helpers = (() => { 
    ...
    return { processFields, findName, toChat, toArray, emproper, splitHandlebars, camelise }
  })();
  ...
  const defaultCliOptions = [
    ...
    },
    {
      ...
      requiredServices: { config: 'ConfigController', buttons: 'ButtonController' },
      action: function() {
        const removableButtons = this.buttons.getButtonNames({ shown: true }),
    ...
    },
    {
      ...
      action: function (args) {
        ...
        if (buttonData && buttonData.name) {
          ...
          if (this.buttons.getButtonNames().includes(buttonName)) return `Invalid button name, already in use: "${buttonName}"`;
          ...
          if (newButton && newButton.math) {
            const { success, msg, err } = this.buttons.addButton(newButton);
    ...
    },
    {
      ...
      action: function (args) {
        ...
        if (buttonData && buttonData.name) {
          buttonData.name = /\s/.test(buttonData.name) ? helpers.camelise(buttonData.name) : buttonData.name;
          if (buttonData.math) {
            const { msg, err } = this.buttons.editButton(buttonData);
            if (msg) helpers.toChat(new ChatDialog({ content: msg, title: 'Edited Button' }, 'default' ));
            else helpers.toChat(new ChatDialog({ content: err }, 'error' ));
          }
        }
      }
    },
    {
      ...
      action: function (args) {
        ...
        if (res) {
          this.cli.modifyConfigArray(args, 'enabledButtons', 0);
    ...
    },
    {
      ...
      requiredServices: {  },
      action: function(args) { this.cli.toggle('ignoreAPI', args) }
    },
    {
      name: 'overheal',
      rx: /^overh/i,
      ...
      requiredServices: {  },
      action: function (args) { this.cli.toggle('overheal', args) }
    },
    {
      name: 'overkill',
      rx: /^overk/i,
      description: `Allow healing to push hp above hpMax`,
      requiredServices: {  },
      action: function (args) { this.cli.toggle('overkill', args) }
    },
    {
      name: 'gmOnly',
      rx: /^gmo/i,
      ...
      requiredServices: {  },
      action: function (args) { this.cli.toggle('gmOnly', args) }
    },
    {
      name: 'settings',
      rx: /^setting/i,
      description: `Open settings UI`,
      action: function() { helpers.toChat(`Not yet implemented.`) }
    },
    {
      name: 'help',
      rx: /^(\?$|h$|help)/i,
 ...
 */
  ...
  class ButtonController {
    ...
    verifyButtons() {
      ...
      if (validButtons.length !== currentButtons.length) {
        ...
        const chatMsg = success && msg ? new ChatDialog({ content: msg, title: 'Buttons Changed' }, 'default') : err ? new ChatDialog({ content: err }, 'error') : null;
        if (chatMsg) helpers.toChat(chatMsg);
      }
    }
  }
  ...
  class CommandLineInterface {
      ...
      commandArray.forEach(command => {
        ...
        for (let option in this._options) {
          if (this._options[option].rx.test(cmd)) {
            ...
            if (success && msg) changed.push(msg);
    ...
    toggle(settingPath, args) {
      ...
      if (newVal != null && this._services.config.changeSetting(settingPath, newVal)) return `${settingPath} set to "${rx.on.test(this._services.config.getSetting(settingPath)) ? 'on' : 'off'}"`;
      else helpers.toChat(`Failed to set ${settingPath}: "${args}"`);
    }
    ...
    //   const currentSetting = this._services.config.getSetting(targetSettingPath);
    ...
    //       else return { err: `Value "${value}" already exists in "${targetSettingPath}"` }
    //     }
    //   } else {
    //     if (currentSetting.includes(value)) newSetting = currentSetting.filter(v => v !== value);
    //     else return { err: `Value "${value}" does not exist in "${targetSettingPath}"` }
    //   }
    //   return this._services.config.changeSetting(targetSettingPath, newSetting);
    // }
  }
    ...
    static _templates = {
      ...
      default: ({ title, content }) => {
        const msgArray = content ? helpers.toArray(content) : [],
          body = msgArray.map(row => `{{${row}}} `);
        return `&{template:default} {{name=${title||scriptName}}} ${body}`;
      },
      error: ({ title, content }) => {
        const errArray = content ? helpers.toArray(content) : [];
        return `
          <div class="error" style="${styles.list.container} border: 3px solid red">
            <div class="error-header" style="${styles.list.header} font-weight: bold">${title}></div>
            <div class="error-body" style="${styles.list.body} border-color: red">${errArray.join('<br>')}</div>
          </div>`
      },
